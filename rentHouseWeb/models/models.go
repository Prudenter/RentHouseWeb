...
func (this *House) To_house_info() interface{} {
	house_info := map[string]interface{}{
		...
		"img_url":     utils.SpliceUrl(this.Index_image_url),
		"room_count":  this.Room_count,
		"order_count": this.Order_count,
		"address":     this.Address,
		"user_avatar": utils.SpliceUrl(this.User.Avatar_url),
...
func (this *House) To_one_house_desc() interface{} {
	...
	for _, img_url := range this.Images {
		img_urls = append(img_urls, utils.SpliceUrl(img_url.Url))
	}
...
func (this *OrderHouse) To_order_info() interface{} {
	order_info := map[string]interface{}{
		...
		"img_url":    utils.SpliceUrl(this.House.Index_image_url),
...
func init() {
	...
	//连接数据   ( 默认参数 ，mysql数据库 ，"数据库的用户名 ：数据库密码@tcp("+数据库地址+":"+数据库端口+")/库名？格式",默认参数-最大空闲连接,最大数据库连接-30）
	orm.RegisterDataBase("default", "mysql", "root:123456@tcp("+utils.G_mysql_addr+":"+utils.G_mysql_port+")/"+utils.G_mysql_dbname+"?charset=utf8", 30)
